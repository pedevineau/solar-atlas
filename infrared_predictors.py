...
def infrared_outputs(times, latitudes, longitudes, temperatures, content_infrared, satellite_step, slot_step,
    ...
    if output_level == 'cli':
        return get_cloud_index(cos_zen=np.cos(angles), mir=content_infrared[:, :, :, nb_channels - 1],
                               ...
                               method='mu-normalization'), cli_eps, mask_input
        ...
        return infrared_abstract_predictors(angles, lir, mask_input, temperatures, cli_eps, difference, satellite_step,
                                            slot_step, gray_scale)


def infrared_abstract_predictors(zen, lir, mask_input, temperatures, cli, ancillary_cloud_index, satellite_step,
                                 slot_step, gray_scale):
    """
...
def get_cloud_index(cos_zen, mir, lir, mask=None, pre_cloud_mask=None, method='default'):
    '''
...
def get_cloud_short_variability(cloud_index, valid_measurements_mask):
    ...
    """
    return compute_variability(cloud_index=cloud_index, mask=valid_measurements_mask, step=1)
...
def get_cloud_index_positive_variability_7d(cloud_index, definition_mask, satellite_step, pre_cloud_mask=None,
    ...
    if only_past:
        if nb_days >= 2:
            var_cli_1d_past = compute_variability(cloud_index=cloud_index, mask=mask,
                                                  step=slots_per_day)
            to_return[slots_per_day:] = var_cli_1d_past[slots_per_day:]

            if nb_days >= 3:
                var_cli_2d_past = compute_variability(cloud_index=cloud_index, mask=mask,
                ...
                if nb_days == 3:
                    ...
                    if nb_days >= 5:
                        ...
                        var_cli_3d_past = compute_variability(cloud_index=cloud_index, mask=mask,
                                                              step=slots_per_day * 3)

                        var_cli_3d_future = compute_variability(cloud_index=cloud_index, mask=mask,
                                                                step=-slots_per_day * 3)
        ...
        if nb_days >= 2:
            ...
            var_cli_1d_future = compute_variability(cloud_index=cloud_index, mask=mask,
                                                    step=-slots_per_day)
            if nb_days == 2:
                ...
                var_cli_2d_past = compute_variability(cloud_index=cloud_index, mask=mask,
                                                      step=slots_per_day * 2)
                var_cli_2d_future = compute_variability(cloud_index=cloud_index, mask=mask,
...
def get_cloud_index_total_variability_7d(cloud_index, definition_mask, pre_cloud_mask,
    ...
    if nb_days >= 2:
        var_cli_1d_past = compute_variability(cloud_index=cloud_index, mask=mask, abs_value=True,
                                              step=nb_slots_per_day)

        var_cli_1d_future = compute_variability(cloud_index=cloud_index, mask=mask, abs_value=True,
                                                step=-nb_slots_per_day)
        if nb_days == 2:
            to_return[:nb_slots_per_day] = var_cli_1d_future[:nb_slots_per_day]
            to_return[nb_slots_per_day:] = var_cli_1d_past[nb_slots_per_day:]
        else:  # nb_days >=3
            var_cli_2d_past = compute_variability(cloud_index=cloud_index, mask=mask, abs_value=True,
                                                  step=nb_slots_per_day * 2)
            var_cli_2d_future = compute_variability(cloud_index=cloud_index, mask=mask, abs_value=True,
            ...
            # last day
            to_return[-nb_slots_per_day:] = np.minimum(var_cli_1d_past[-nb_slots_per_day:],
            ...
            if nb_days == 3:
                ...
                if nb_days == 5:
                    ...
                    var_cli_3d_past = compute_variability(cloud_index=cloud_index, mask=mask, abs_value=True,
                                                          step=nb_slots_per_day * 3)

                    var_cli_3d_future = compute_variability(cloud_index=cloud_index, mask=mask, abs_value=True,
                                                            step=-nb_slots_per_day * 3)
                    # two days previous the last one
                    ...
                    if nb_days >= 7:
                        to_return[3 * nb_slots_per_day:-3 * nb_slots_per_day] = np.minimum(
                            np.minimum(
                                np.minimum(
