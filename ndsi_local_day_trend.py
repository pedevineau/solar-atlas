...
def recognize_pattern_ndsi(ndsi, mu, mask, mask_high_variability,
    ...
    nb_steps = int(
        np.ceil(nb_slots / nb_slots_per_step)) + 1  # +1 because first slot is not the darkest slot for every point
                        ...
                        if slice_ndsi[~slice_mask].size > minimal_nb_unmasked_slots:
                            ...
                            if True:
                                ...
                                if r_value > 1 - tolerance:
                                    persistence_array[
                                        step, lat, lon] = 1  # maximum(med+0.4, 1.) # med of 0.6 is considered as snow-like with p=1
