...
def row_mtx_fxed(GHI,DHI,DNI,DiffHor,lati,a0,h0refr,sina0,cosa0,sinh0,cosh0,sinDeltaexp,sinGammaN,cosGammaN,\
                        sinTiltSN,cosTiltSN, module_x,module_y,spacing_y,shadow,vert_sect=11,hor_sect=3,Albedo=0.11,AlterAzimuth=False):
    ...
    for iy in range(0,lenY): 
        for ix in range(0,lenX):
            DiffInc_M[:,iy,ix],ReflInc_M[:,iy,ix]=solar_geom_v5.diffinc_PerezFast(DiffHor_M[:,iy,ix],DNI_M[:,iy,ix],DHI_M[:,iy,ix],h0refr,sinGammaN*np.ones_like(a0),\
    ...
    DiffInc,ReflInc = solar_geom_v5.diffinc_PerezFast(DiffHor,DNI, DHI,h0refr,sinGammaN*np.ones_like(a0),cosGammaN*np.ones_like(a0),Albedo,sinDeltaexp, shadow, \
...
def row_mtx_1xNS(GHI,DHI,DNI,DiffHor,lati_r,a0_r,h0refr_r,sina0,cosa0,sinh0,cosh0,sinDeltaexp,tilt,sinGammaNV,cosGammaNV,\
                        sinrot,cosrot,module_x,module_y,spacing_y,spacing_x,shadow=None, Albedo=Albedo,AlterAzimuth=False):
...
def irrad2loss(EnergyDirect_MTX,EnergyDiffuse_MTX,EnergyDirectMax,EnergyDiffuseMax,BypassLoss):
        ...
        lossEle=((meanDiffuse+meanDirect-(meanDirect-minDirect)*BypassLoss)/(maxDiffuse+maxDirect))
...
def calculate_shadow_losses(mounting,terrainSlope, terrainAzimuth, rel_spacing_rows,rel_spacing_columns, BypassLoss, GHI, DHI, DNI, DiffHor,Refl,\
                            lati, a0, h0refr, sina0, cosa0, sinh0, cosh0, tilt, sinGNV, cosGNV, sinDeltaexp, sinrot,cosrot,shadow, Albedo=0.11,AlterAzimuth=False):    
...
#no inclination
def apply_shading(ghi_sat, dni_sat, h0_sat, shading_data_dict, Albedo=0.12):
...
def apply_shading_inplane(ghi_sat, dni_sat, h0_sat, a0_sat , shading_data_dict, mounting, aspect, inclination, latit, Albedo=0.12, radiation_config_dict=None, pvParamDict=None):
    ...
    if (radiation_config_dict is not None) and (pvParamDict is not None):
        ...
        mounting, modul_asp, modul_incl, terrainSlope,  terrainAzimuth, do_interrow_shading,  do_backtrack, relative_spacing_columns, relative_spacing_rows, rotation_limit, rotation_limit2 = result
        ...
        mounting_geom.mounting_geom_angles(mounting, sina0, cosa0, a0, sinh0, cosh0, h0, GN=modul_incl, AN=modul_asp, latitude=latit, rotation_limit=rotation_limit, rotation_limit2=rotation_limit2,  backtrack=do_backtrack, relative_spacing_rows=relative_spacing_rows, relative_spacing_columns=relative_spacing_columns )
    else:
        modul_incl = np.radians(inclination) 
        modul_asp = np.radians(aspect)
        #calculate module geometry
        incidence_angle, cos_incidence_angle, GammaNV, sinGammaNV,cosGammaNV,ANV, sinANV, cosANV ,rot, sinrot, cosrot=\
        mounting_geom.mounting_geom_angles(mounting, sina0, cosa0, a0, sinh0, cosh0, h0, GN=modul_incl, AN=modul_asp, latitude=latit, rotation_limit=[-np.pi*2.,np.pi*2.], rotation_limit2=[-np.pi,np.pi], backtrack=False, relative_spacing_rows=1e10, relative_spacing_columns=1e10)
    ...
    #        result = mounting_geom.PV_extract_mounting_details(radiation_config_dict, pvParamDict)
    #        mounting, modul_asp, modul_incl, terrainSlope,  terrainAzimuth, do_interrow_shading,  do_backtrack, relative_spacing_columns, relative_spacing_rows, rotation_limit, rotation_limit2 = result
    #        incidenceAngle, GammaNV, dummy, inc_angle_cos_grid, sinGammaN,cosGammaN, dummy, dummy = mounting_geom.mounting_geom_angles(mounting, np.radians(a0_sat[wh]), h0_sat_r, GN=modul_incl, AN=modul_asp, latitude=latit, rotationRange=rotation_limit_r, tiltRange=rotation_limit2_r,  backtrack=do_backtrack, relativeSpacingRows=relativeSpacingRows, relativeSpacingColumns=relativeSpacingColumns )
    #    else:
    #        #old implementation
    #        modul_incl, modul_asp = np.radians(inclination), np.radians(aspect)
    #        incidenceAngle, GammaNV, dummy, inc_angle_cos_grid, sinGammaN,cosGammaN, dummy, dummy = mounting_geom.mounting_geom_angles(mounting, np.radians(a0_sat[wh]), h0_sat_r, GN=modul_incl, AN=modul_asp, latitude=latit)
...
def apply_noshading_inplane(ghi_sat, dni_sat, h0_sat, a0_sat , shading_data_dict, mounting, aspect, inclination, latit, Albedo=0.12, radiation_config_dict=None, pvParamDict=None):
