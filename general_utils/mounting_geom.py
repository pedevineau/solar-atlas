...
def optPhiB(sina0,cosa0,sinh0,cosh0,sinpsi,cospsi):
    phiopt=np.arctan2((sina0*cosh0),(cosa0*cosh0*sinpsi+sinh0*cospsi))
    sinphiopt=np.sin(phiopt)
    cosphiopt=np.cos(phiopt)
...
def optPhiC(sina0,cosa0,sinh0,cosh0):
    phiopt=np.arctan2(cosa0,(sinh0/cosh0))
    cosphiopt=np.cos(phiopt)
    sinphiopt=np.sin(phiopt)
    return sinphiopt,cosphiopt
...
def NS_TrackerBacktrack_simple(relative_spacing_columns, phioptB):
    a=relative_spacing_columns*np.cos(phioptB)
    a[a>1]=1
    a[a<-1]=-1
    phioptB_corrector=np.arccos(a)
    phioptB_corrected=phioptB
    phioptB_corrected[phioptB>0]=phioptB[phioptB>0]-phioptB_corrector[phioptB>0]
    phioptB_corrected[phioptB<0]=phioptB[phioptB<0]+phioptB_corrector[phioptB<0]

    return phioptB_corrected


def Tracker_1x_NS(latitude,a0,sina0,cosa0,h0,sinh0,cosh0,GN,sinGN,cosGN, rotation_limit,rotation_limit2,relative_spacing_columns,relative_spacing_rows,backtrack):
    ...
    sinphioptB,cosphioptB=optPhiB(sina0,cosa0,sinh0,cosh0,sinGammaNV*lat_sign,cosGammaNV)
    ...
    sinANV=np.ones_like(a0)*np.sign(a0)
    cosANV=np.zeros_like(a0)
    ANV=np.arcsin(sinANV)
...
def Tracker_2x(latitude,a0,sina0,cosa0,h0,sinh0,cosh0,GN,sinGN,cosGN, rotation_limit,rotation_limit2,relative_spacing_columns,relative_spacing_rows,backtrack):
    ...
    # print 'DEBUG',rotation_limit
    if rotation_limit is None:
        rotation_limit=[np.radians(-180),np.radians(180)]
    if rotation_limit2 is None:
        rotation_limit2=np.array((0,np.radians(180)))
...
def PV_extract_mounting_details(rad_configDict, pvDict):    
    ...
    rotation_limit=np.array([-2*np.pi,2*np.pi])
    ...
    if pvDict.has_key("pvTrackerRotMin"):
        rotation_limit=np.radians(pvDict['pvTrackerRotMin'])
    if pvDict.has_key("pvTrackerRot2Min"):
        rotation_limit2=np.radians(pvDict['pvTrackerRot2Min'])
