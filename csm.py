        ...
        for file_ in (i for i in os.listdir(dir_in)):
            ...
            try:
                with Dataset(file_path) as ncF:
                    ...
                    csp_data = csp_[:]
                    if is_range_array(csp_data, bmin=0.0, bmax=1.0):
                        newcsp = Dataset(
                            os.path.join(
                                dir_csp_out,
                                file_.replace(
                                    "P1S-ABOM_CLD-PRJ_GEOS141_2000", "CSP_LATLON"
                                ),
                            ),
                            "w",
                            "NETCDF4",
                        )
                        rectl, rectc = proj4_navigation.ll2lc(
                            lats,
                            lons,
                            ssp=ssp,
                    ...
                    ct_data = ct_[:]
                    if is_range_array(ct_data, bmin=0.0, bmax=10.0):
                        newct = Dataset(
                            os.path.join(
                                dir_ct_out,
                                file_.replace(
                                    "P1S-ABOM_CLD-PRJ_GEOS141_2000", "CT_LATLON"
                                ),
                            ),
                            "w",
                            "NETCDF4",
        ...
        for file_ in (i for i in os.listdir(dir_in)):
            ...
            try:
                with Dataset(file_path) as ncF:
                    ...
                    csp_ = ncF.variables[nc_csp]
                    csp_data = csp_[:].data
                    if is_range_array(csp_data, bmin=-1.0, bmax=1.0):
                        ...
                        minu = rad[9:11]
                        str_date = (
                            doyy2yyyymmdd(int(doy), int(year)) + hour + minu + "00"
                        )
                        str_date = str_date + "-CSP_LATLON-GOES16.nc"
                        newcsp = Dataset(
                            os.path.join(dir_csp_out, str_date), "w", "NETCDF4"
                        )
                        rectl, rectc = abi_navigation.ll2lc(
                            lats, lons, lon0=lon0, res_km=2
                        )
