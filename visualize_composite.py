...
def get_snow_composite(latitudes, longitudes, dfb_beginning, dfb_ending, slot_step):
    ...
    mir = get_features('infrared', latitudes, longitudes, dfb_beginning, dfb_ending, False, slot_step)[:, :, :, 1] - 220
    (nb_slots, nb_latitudes, nb_longitudes) = np.shape(vis)
    vis[vis < 0] = 0
    nir[nir < 0] = 0
    mir[mir < 0] = 0
    mir = normalize_array(mir, normalization='max')
    composite = np.empty((nb_slots, nb_latitudes, nb_longitudes, 3))
    composite[:, :, :, 0] = vis
    composite[:, :, :, 1] = nir
    composite[:, :, :, 2] = mir
    return 256*composite
