...
def classify_brightness(bright_index):
    ...
    better_than_threshold = (
        centers3[bright, 0] + get_std(model, process, bright) / 2
    ) > default_threshold
    separability_condition = centers3[bright, 0] - centers3[dark, 0] > 1.2 * max(
        get_std(model, process, dark), get_std(model, process, bright)
    )
    ...
    if not better_than_threshold or not separability_condition:
        ...
        if nb_components >= 4:
            in_between = (
                nb_components * (nb_components - 1) / 2 - dark - bright - undefined
            )
...
def classifiy_brightness_variability(bright_variability):
    ...
    bright_variability_1d = bright_variability.reshape(
        nb_slots * nb_latitudes * nb_longitudes
    )
    bright_variability_1d = bright_variability_1d[
        ~np.isnan(bright_variability_1d)
    ].reshape(-1, 1)
    training_rate = 0.005
    ...
    model = get_basis_model(process, nb_components, max_iter, means_init)
    model = get_trained_model(bright_index_training, model, process)
    brightness_variability = model.predict(bright_variability_1d).reshape(
        (nb_slots, nb_latitudes, nb_longitudes)
    )
...
def check_gaussian_hypothesis(latitudes, longitudes, begin, end, method="none"):
    ...
    snow = get_features("visible", latitudes, longitudes, begin, end, "abstract")[
        :, :, :, 0
    ]
    # snow=feat[:,:,:,0]
    # var=feat[:,:,:,1]
    ...
    dmask = dawn_day_test(
        get_zenith_angle(
            get_times_utc(begin, end, read_satellite_step(), 1), latitudes, longitudes
        )
    )
    if method == "tomas":
        cloud = (
            reduce_tomas_2_classes(
                get_tomas_outputs(
                    begin,
                    end,
                    latitudes[0],
        ...
        classes = reduce_classes(
            get_classes_v1_point(latitudes, longitudes, begin, end)
        )
