...
def write(
    ...
    try:
        ds = DataSet.create(
            ...
            globals_definition=dict(
                DC.CF16_DEFAULT_GLOBALS_DEFINITION,
                **{
                    "title": "Computed visible indexes",
                    "source": "PED",
                    "comments": "For improved visualization",
                }
            ),
...
if __name__ == "__main__":
    ...
    latitude_beginning = 40.0  # salt lake mongolia  45.
    latitude_end = 45.0
    ...
    dfbs, slots = get_dfbs_slots(
        dfb_beginning, dfb_ending, satellite_timestep, slot_step_
    )
    ...
    if type_output == "infrared":
        ...
        big_clouds = np.reshape(
            big_clouds, (nb_dfbs, nb_slots_per_day, nb_latitudes, nb_longitudes)
        )
        var_difference = np.reshape(
            var_difference, (nb_dfbs, nb_slots_per_day, nb_latitudes, nb_longitudes)
        )
        warm = np.reshape(
            warm, (nb_dfbs, nb_slots_per_day, nb_latitudes, nb_longitudes)
        )
        cold = np.reshape(
            cold, (nb_dfbs, nb_slots_per_day, nb_latitudes, nb_longitudes)
        )
        ...
        variables_definitions_cli = {
            "Big_clouds": {
                "_FillValue": -999.0,
                "units": "no unit",
                "long_name": "mask with hot water clouds",
                "datatype": "f8",
            ...
            "Var_difference": {
                "_FillValue": -999.0,
                "units": "no unit",
                "long_name": "5 days variability difference (for small hot clouds)",
                "datatype": "f8",
                "cell_methods": "time: mean (interval: 1 day comment: hourly sum averages) latitude: mean longitude: mean",
                "grid_mapping": "coordinate_reference_system",
                "dimensions": ("dfb", "slot", "latitude", "longitude"),
    ...
    if type_output == "visible":
        ...
        ndsi = np.reshape(
            ndsi, (nb_dfbs, nb_slots_per_day, nb_latitudes, nb_longitudes)
        )
        var_ndsi = np.reshape(
            var_ndsi, (nb_dfbs, nb_slots_per_day, nb_latitudes, nb_longitudes)
        )
        cloudy_sea = np.reshape(
            cloudy_sea, (nb_dfbs, nb_slots_per_day, nb_latitudes, nb_longitudes)
        )
        ...
        variables_definitions_ndsi = {
            ...
            "Var_snow_index": {
                "_FillValue": -999.0,
                "units": "no unit",
                "long_name": "5 days variability of snow index",
                "datatype": "f8",
                "cell_methods": "time: mean (interval: 1 day comment: hourly sum averages) latitude: mean longitude: mean",
                "grid_mapping": "coordinate_reference_system",
                "dimensions": ("dfb", "slot", "latitude", "longitude"),
    ...
    if type_output == "classes":
        classes = get_classes_v1_point(
            latitudes, longitudes, dfb_beginning, dfb_ending, slot_step=slot_step_
        )

        classes = np.flip(classes, axis=1)

        classes = np.reshape(
            classes, (nb_dfbs, nb_slots_per_day, nb_latitudes, nb_longitudes)
        )
        ...
        variables_definitions_classes = {
            "Classes": {
                "_FillValue": -999.0,
                "units": "no unit",
                "long_name": "Decision tree classification",
                "datatype": "f8",
                "cell_methods": "time: mean (interval: 1 day comment: hourly sum averages) latitude: mean longitude: mean",
                "grid_mapping": "coordinate_reference_system",
                "dimensions": ("dfb", "slot", "latitude", "longitude"),
