...
def classify_cloud_variability(cloud_variability):
    ...
    :return: array slots*latitudes*longitudes with 1 for slightly_cloudy, 0 for cloudless, and 2 for undetermined (slight covertness)
    '''
    (nb_slots, nb_latitudes, nb_longitudes) = np.shape(cloud_variability)[0:3]
    cloud_variability = cloud_variability.reshape(nb_slots * nb_latitudes * nb_longitudes).reshape(-1, 1)
    training_rate = 0.005
